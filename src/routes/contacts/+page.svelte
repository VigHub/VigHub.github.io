	...
	<div class="w-[80%]">
		<form>
			...
			<label class="label m-3">
				<span>From</span>
				<input
					class="block w-full rounded-3xl bg-transparent"
					type="text"
					placeholder="Your email"
					bind:value={$mail.value}
				/>
				{#if $emailForm.hasError('mail.required') && clicked > 0}
					<div class="text-red-500">Your email is required</div>
				{/if}
				{#if $emailForm.hasError('mail.not_an_email') && clicked > 0}
					<div class="text-red-500">Write a valid email address</div>
				{/if}
			</label>
			<label class="label m-3">
				<span>Subject</span>
				...
				{#if $emailForm.hasError('subject.required') && clicked > 0}
					<div class="text-red-500">Subject of your email is required</div>
				{/if}
			</label>
			<label class="label m-3">
				<span>Text</span>
				<textarea
					class="block w-full bg-transparent rounded-2xl"
					rows="4"
					placeholder="Tell me what you want!"
					bind:value={$emailContent.value}
				/>
				{#if $emailForm.hasError('emailContent.required') && clicked > 0}
					<div class="text-red-500">Don't you want to write me anything?</div>
				{/if}
			</label>
			<button
				on:click={handleSubmit}
				class="btn bg-gray-800 text-white dark:bg-gray-100 dark:text-black float-right">Send</button
			>
		</form>
		<Toast />
	</div>
